 ...
 */
const CACHE_NAME = 'wnzyq-v51'; // R41：封面图没填链接时预览区也显示感叹号占位符（与加载失败一致） // R40：新增类型按钮与「上传本地图片」按钮完全同款（纯蓝底/13px/8px圆角/40px高，除文字外逐项对齐实测值） // R39：退出确认弹窗打开期间退出键变白（复用分享键 .active 白底蓝字+弹窗关闭同步移除） // R35：翻页组件两组不拆散（连带修复无限滚动误清组盒按钮）；导航页按钮回退组合居中；手机网格卡片操作按钮裁剪修复
const STATIC_ASSETS = [
  './',
  './index.html',
  './shop.html',
